...
namespace nd::src::graphics::vulkan
{
    ...
    getNextImageIndex(const VkDevice       device,
                      ...
                      const VkFence        fence,
                      const u64            timeout) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 index;

        ND_VK_ASSERT(vkAcquireNextImageKHR(device, swapchain, timeout, semaphore, fence, &index));

        return index;
    }

    vec<Image>
    getSwapchainImages(const VkDevice device, const VkSwapchainKHR swapchain) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 count;

        ND_VK_ASSERT(vkGetSwapchainImagesKHR(device, swapchain, &count, nullptr));

        auto images = vec<Image>(count);

        ND_VK_ASSERT(vkGetSwapchainImagesKHR(device, swapchain, &count, images.data()));

        return images;
    }

    vec<VkSurfaceFormatKHR>
    getSurfaceFormats(const VkPhysicalDevice physicalDevice, const VkSurfaceKHR surface) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 count;

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &count, nullptr));

        auto formats = vec<VkSurfaceFormatKHR>(count);

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &count, formats.data()));

        return formats;
    }

    vec<VkPresentModeKHR>
    getSurfacePresentModes(const VkPhysicalDevice physicalDevice, const VkSurfaceKHR surface) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 count;

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &count, nullptr));

        auto presentModes = vec<VkPresentModeKHR>(count);

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &count, presentModes.data()));

        return presentModes;
    }

    VkSurfaceCapabilitiesKHR
    getSurfaceCapabilities(const VkPhysicalDevice physicalDevice, const VkSurfaceKHR surface) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        VkSurfaceCapabilitiesKHR capabilities;

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface, &capabilities));
    ...
    isSurfaceQueueFamilySupported(const VkPhysicalDevice physicalDevice,
                                  ...
                                  const QueueFamily&     queueFamily) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        VkBool32 supported;

        ND_VK_ASSERT(vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice, queueFamily.index, surface, &supported));
    ...
    Swapchain
    createSwapchain(opt<const SwapchainCfg>::ref cfg, const VkDevice device) noexcept(ND_VK_ASSERT_NOTHROW&& ND_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
        ...
        ND_VK_ASSERT(vkCreateSwapchainKHR(device, &createInfo, ND_VK_ALLOCATION_CALLBACKS, &swapchain));
    ...
    createSwapchainImageViews(opt<const ImageViewCfg>::ref cfg,
                              ...
                              const vec<VkImage>&          swapchainImages) noexcept(ND_VK_ASSERT_NOTHROW&& ND_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
    ...
    createSwapchainFramebuffers(opt<const FramebufferCfg>::ref cfg,
                                const VkDevice                 device,
                                const vec<VkImageView>&        swapchainImageViews) noexcept(ND_VK_ASSERT_NOTHROW&& ND_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
