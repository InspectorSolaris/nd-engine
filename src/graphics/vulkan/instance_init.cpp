...
namespace nd::src::graphics::vulkan
{
    ...
    getInstanceLayerProperties() noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ...
        ND_VULKAN_ASSERT_EXEC(vkEnumerateInstanceLayerProperties(&count, nullptr));

        auto properties = vec<VkLayerProperties>(count);

        ND_VULKAN_ASSERT_EXEC(vkEnumerateInstanceLayerProperties(&count, properties.data()));
    ...
    getInstanceExtensionProperties() noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ...
        ND_VULKAN_ASSERT_EXEC(vkEnumerateInstanceExtensionProperties(nullptr, &count, nullptr));

        auto properties = vec<VkExtensionProperties>(count);

        ND_VULKAN_ASSERT_EXEC(vkEnumerateInstanceExtensionProperties(nullptr, &count, properties.data()));

        return properties;
    }

    bool
    isInstanceLayersSupported(const vec<str>& layers) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ND_SET_SCOPE();
    ...
    bool
    isInstanceExtensionsSupported(const vec<str>& extensions) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ND_SET_SCOPE();
    ...
    Instance
    createInstance(opt<const InstanceCfg>::ref cfg) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW&& ND_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
        ...
        ND_VULKAN_ASSERT_EXEC(vkCreateInstance(&createInfo, ND_VULKAN_ALLOCATION_CALLBACKS, &instance));
