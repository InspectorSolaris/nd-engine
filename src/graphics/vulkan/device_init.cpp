...
namespace nd::src::graphics::vulkan
{
    ...
    getPhysicalDevices(const VkInstance instance) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 count;

        ND_VK_ASSERT(vkEnumeratePhysicalDevices(instance, &count, nullptr));

        auto physicalDevices = vec<VkPhysicalDevice>(count);

        ND_VK_ASSERT(vkEnumeratePhysicalDevices(instance, &count, physicalDevices.data()));

        return physicalDevices;
    }

    vec<VkExtensionProperties>
    getPhysicalDeviceExtensionProperties(const VkPhysicalDevice physicalDevice) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();

        u32 count;

        ND_VK_ASSERT(vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &count, nullptr));

        auto properties = vec<VkExtensionProperties>(count);

        ND_VK_ASSERT(vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &count, properties.data()));
    ...
    bool
    isPhysicalDeviceExtensionsSupported(const VkPhysicalDevice physicalDevice, const vec<str>& extensions) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
    ...
    PhysicalDevice
    getPhysicalDevice(opt<const PhysicalDeviceCfg>::ref cfg, const VkInstance instance) noexcept(ND_VK_ASSERT_NOTHROW&& ND_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
    ...
    Device
    createDevice(opt<const DeviceCfg>::ref cfg, const VkPhysicalDevice physicalDevice) noexcept(ND_VK_ASSERT_NOTHROW)
    {
        ND_SET_SCOPE();
        ...
        ND_VK_ASSERT(vkCreateDevice(physicalDevice, &createInfo, ND_VK_ALLOCATION_CALLBACKS, &device));
