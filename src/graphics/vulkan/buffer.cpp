...
namespace nd::src::graphics::vulkan
{
    ...
    Buffer::Buffer(const VkDevice device, const CreateInfo &createInfo) : device_(device)
    {
        ND_SET_SCOPE_LOW();
    ...
    Buffer::Buffer(Buffer &&buffer) noexcept : device_(std::move(buffer.device_)), buffer_(std::move(buffer.buffer_))
    {
        ND_SET_SCOPE_LOW();
