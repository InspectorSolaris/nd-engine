...
namespace nd::src::graphics::vulkan
{
    ...
    DescriptorPool
    createDescriptorPool(opt<const DescriptorPoolCfg>::ref cfg, const VkDevice device) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ND_SET_SCOPE();
        ...
        ND_VULKAN_ASSERT_EXEC(vkCreateDescriptorPool(device, &createInfo, ND_VULKAN_ALLOCATION_CALLBACKS, &descriptorPool));

        return descriptorPool;
    }

    DescriptorSetLayout
    createDescriptorSetLayout(opt<const DescriptorSetLayoutCfg>::ref cfg, const VkDevice device) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ND_SET_SCOPE();
        ...
        ND_VULKAN_ASSERT_EXEC(vkCreateDescriptorSetLayout(device, &createInfo, ND_VULKAN_ALLOCATION_CALLBACKS, &descriptorSetLayout));

        return descriptorSetLayout;
    }

    DescriptorSetLayoutObjects
    createDescriptorSetLayoutObjects(opt<const DescriptorSetLayoutObjectsCfg>::ref cfg, const VkDevice device) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ND_SET_SCOPE();
                           ...
                           const VkDevice                   device) noexcept(ND_VULKAN_ASSERT_EXEC_NOTHROW)
    {
        ...
        ND_VULKAN_ASSERT_EXEC(vkAllocateDescriptorSets(device, &allocateInfo, descriptorSets.data()));
